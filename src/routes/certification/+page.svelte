<script lang="ts">
	...
	import CertificationCard from '$lib/components/certification/certification-card.svelte';
	import Icon from '$lib/components/ui/icon/icon.svelte';
	import Toggle from '$lib/components/ui/toggle/toggle.svelte';
	import CertificationData from '$lib/data/certification';
	...
	let filters: Array<SkillFilter> = $state(
		SkillsData.items.filter((it) => {
			return CertificationData.items.some((certification) =>
				certification.skills.some((skill) => skill.slug === it.slug)
			);
		})
	);
	...
	let search = $state('');
	let result = $derived(
		CertificationData.items.filter((certification) => {
			const isFiltered =
				filters.every((item) => !item.isSelected) ||
				certification.skills.some((tech) =>
					filters.some((filter) => filter.isSelected && filter.slug === tech.slug)
				);

			const isSearched =
				search.trim().length === 0 ||
				certification.name.trim().toLowerCase().includes(search.trim().toLowerCase());
...
<SearchPage title={CertificationData.title} {onSearch}>
		...
		{:else}
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
				{#each result as it (it.slug)}
					<CertificationCard certification={it} />
				{/each}
			</div>
		{/if}
	</div>
